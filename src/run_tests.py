...
def GetTestCommand(test_type, test_file):
    return './lzw_test --type {} --file {}'.format(test_type, test_file)

def TestAndPrintStatus(prefix, test_type, test_file, expected_count):
    cmd = GetTestCommand(test_type, test_file)
    ...
    if test_type == 'encode' or test_type == 'decode':
        extra_switch = '--type {}'.format(test_type)

    res = 0
    if ok and tests_run != expected_count:
        print '{}: {}FAILED{} not all tests completed'.format(prefix, BColors.FAIL, BColors.ENDC)
        print '\tto repeat only last test run ./lzw_test --file {} --test_index {} {}'.format(test_file, last_test_index, extra_switch)
        print '\tto extract input for last test run ./lzw_test --file {} --test_index {} {} --dump'.format(test_file, last_test_index, extra_switch)
        res = 0
    elif ok:
        ...
        print '\tto repeat only this test run ./lzw_test --file {} --test_index {} {}'.format(test_file, last_test_index, extra_switch)
        print '\tto extract input for test run ./lzw_test --file {} --test_index {} {} --dump'.format(test_file, last_test_index, extra_switch)
        res = 0
